...
public class OAuthSignature {
    ...
    private static final String BASIC_SIGNATURE_FORMAT = "%s=%s&%s=%s&%s";
    ...
    public static String getAsQueryString(OAuthConfig config, String endpoint, HttpMethod httpMethod, Map<String, String> params) {
        ...
        for (Map.Entry<String, String> entry : params.entrySet()) {
            // Append key=value to the result string
        ...
        // Remove the trailing "&" if there is at least one entry
        if (!params.isEmpty()) {
            result.deleteCharAt(result.length() - 1);
        }
        String base_params =result.toString();
        String s =  String.format(BASIC_SIGNATURE_FORMAT, OAuthHeader.OAUTH_CONSUMER_KEY.getValue(), config.getConsumerKey(), OAuthHeader.OAUTH_CONSUMER_SECRET.getValue(),config.getConsumerSecret(),base_params);
        return s;
