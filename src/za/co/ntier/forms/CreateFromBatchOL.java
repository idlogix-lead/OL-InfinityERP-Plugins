...
public abstract class CreateFromBatchOL extends CreateFrom 
{
	/**
	 * 
	 * @param gridTab
	 */
	public CreateFromBatchOL(GridTab gridTab) 
	{
		super(gridTab);
	}
	...
	protected String getSQLWhere(Integer BPartner, String DocumentNo, Timestamp DateFrom, Timestamp DateTo, 
			BigDecimal AmtFrom, BigDecimal AmtTo, Integer DocType, String TenderType, String AuthCode,String poreference)
	{
		...
		if(poreference != null && poreference.length() > 0)
			sql.append(" AND inv.poreference = ? ");
	...
	throws SQLException
	{
		...
		if(poreference != null && poreference.length() > 0)
			pstmt.setString(index++, (poreference));
	}
