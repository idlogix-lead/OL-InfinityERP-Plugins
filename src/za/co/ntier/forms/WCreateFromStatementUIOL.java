...
public class WCreateFromStatementUIOL extends CreateFromStatementOL implements EventListener<Event>
{
	 ...
	 * @param tab
	 */
	public WCreateFromStatementUIOL(GridTab tab) 
	{
		super(tab);
	...
	protected Label poreferenceLabel = new Label();
	protected WStringEditor poreferenceField = new WStringEditor();
	...
	protected void zkInit() throws Exception
	{
    	...
    	poreferenceLabel.setText(Msg.translate(Env.getCtx(), "POReference"));
		...
		row = rows.newRow();
		row.appendChild(poreferenceLabel.rightAlign());
		row.appendChild(poreferenceField.getComponent());
	...
	protected void loadBankAccount()
	{
		loadTableOIS(getBankAccountData((Integer)bankAccountField.getValue(), (Integer)bPartnerLookup.getValue(), 
				...
				(Integer)documentTypeField.getValue(), (String)tenderTypeField.getValue(), authorizationField.getValue().toString(),poreferenceField.getValue().toString()));
	}
